...
if(params['dataset'] == 'MNIST'):
    from models.mnist_aux_model import Generator, Discriminator, DHead, QHead
...
# num_label_c : number of labels given.
if(params['dataset'] == 'MNIST'):
    params['num_z'] = 44 # 62
    params['num_dis_c'] = 0 # 1
    params['dis_c_dim'] = 0 # 10
    params['num_con_c'] = 30 # 2
    params['num_dis_label_c'] = 0
    params['num_con_label_c'] = 0
    params['num_con_c_max'] = 10
elif(params['dataset'] == 'SVHN'):
    params['num_z'] = 124
    params['num_dis_c'] = 4
    params['dis_c_dim'] = 10
...
# Loss for information bottleneck
criterionB_con = BottleneckLoss()
...
beta = 0
# Constraint on mutual information
i_c = params['i_c']
...
if(params['num_dis_label_c'] != 0):
    idx = np.arange(params['dis_c_dim']).repeat(10)
    dis_label_c = torch.zeros(100, params['num_dis_label_c'], params['dis_c_dim'], device=device)
    for i in range(params['num_dis_label_c']):
        dis_label_c[torch.arange(0, 100), i, idx] = 1.0

    dis_label_c = dis_label_c.view(100, -1, 1, 1)

    fixed_noise = torch.cat((fixed_noise, dis_label_c), dim=1)
...
for epoch in range(params['num_epochs']):
    ...
    for i, (data, targets) in enumerate(dataloader, 0):
        ...
        real_targets = targets.to(device)
        ...
        if params['num_dis_label_c'] != 0:
            ...
            noise = torch.cat((noise, dis_label_c), dim=1)
        ...
        # Calculate encoder representation.
        q_logits, q_mu, q_var = netQ(output)
        
        # Updating beta and bottleneck loss
        loss_bottleneck = criterionB_con(q_mu[:,:params['num_con_c_max']], q_var[:,:params['num_con_c_max']], i_c)
        
        # Net Loss for the discriminator
        D_loss = loss_real + loss_fake + beta * loss_bottleneck
        # Update parameters
        optimD.step()
        
        # Update beta.
        beta = optimize_beta(beta, loss_bottleneck)
        ...
        con_loss = 0
        if (params['num_con_c'] != 0):
            start = params['num_z'] + params['num_dis_c'] * params['dis_c_dim'] + params['num_con_c_max']
            con_loss = criterionQ_con(noise[:, start : start + params['num_con_c'] - params['num_con_c_max']].reshape(-1, params['num_con_c'] - params['num_con_c_max']), q_mu[:,params['num_con_c_max']:], q_var[:,params['num_con_c_max']:])*0.1
            
        # Calculating loss for discrete label code.
        dis_label_loss = 0
        for j in range(params['num_dis_label_c']):
            dis_loss -= criterionQ_dis(q_logits[:, j*10 : j*10 + 10], real_targets)
        ...
        if i != 0 and i%100 == 0:
            print('[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tLoss_B: %.4f'
                  % (epoch+1, params['num_epochs'], i, len(dataloader), 
                    D_loss.item(), G_loss.item(), loss_bottleneck))

        # Save the losses for plotting.
        G_losses.append(G_loss.item())
        D_losses.append(D_loss.item())
        B_losses.append(loss_bottleneck)
